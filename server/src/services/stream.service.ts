...
class StreamService {
  ...
  /**
   * TODO: Future Channel API Redesign
   * ──────────────────────────────────────────────────────────────────────
   * The backend channel API will later be redesigned.
   *
   ...
   *   1. Determine the authenticated user (from req.auth.clerkUserId).
   *   2. Look up the Persona by personaId (from a Persona model/collection).
   *   3. Generate the channel ID deterministically (e.g. "persona-{clerkId}-{personaId}").
   *   4. Determine Stream members (the user + any AI bot associated with the Persona).
   *   5. Create or fetch the channel (idempotent).
   *
   * Keep all channel creation logic isolated inside StreamService so this
   * future migration only requires changes in one place — this file and the
   * controller that calls it. No frontend, middleware, or route changes needed.
   * ──────────────────────────────────────────────────────────────────────
   */
